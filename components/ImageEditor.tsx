  ...
  return (
    <div className="h-full w-full bg-neutral-950 flex flex-col text-white">
      ...
      <header className="bg-red-700 shadow-lg z-10 flex-none flex items-center justify-between p-4">
        <button onClick={onBack} className="flex items-center gap-2 text-white hover:bg-red-800 p-2 rounded transition-colors">
          <ArrowLeft className="w-5 h-5" />
          <span className="font-sans font-black italic text-sm uppercase hidden sm:inline">Back</span>
        </button>
        <h2 className="text-xl font-sans font-black italic uppercase tracking-tighter transform -skew-x-6">Edit Photo</h2>
        <button 
          onClick={handleSave} 
        ...
        <div className="h-32 p-4 overflow-y-auto">
          ...
          {activeTab === 'filters' && (
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
              {FILTERS.map(filter => (
                ...
                >
                  <div className={`w-16 h-16 rounded-lg border-2 overflow-hidden ${activeFilter === filter.id ? 'border-red-600 ring-2 ring-red-600/30' : 'border-neutral-600'}`}>
                     {/* Preview thumbnail using the same image but scaled down CSS */}
                     <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${imageSrc})`, filter: filter.filter }}></div>
                  </div>
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${activeFilter === filter.id ? 'text-red-500' : 'text-neutral-400'}`}>
                    {filter.name}
                  </span>
                </button>
              ))}
          ...
          {activeTab === 'crop' && (
            <div className="flex justify-center gap-8 items-center h-full">
               <button onClick={rotate} className="flex flex-col items-center gap-2 text-neutral-300 hover:text-red-500 transition-colors">
                 <div className="p-3 bg-neutral-800 rounded-full">
                    <RotateCw className="w-6 h-6" />
                 </div>
                 <span className="text-[10px] font-bold uppercase">Rotate</span>
               </button>
               <button onClick={flip} className="flex flex-col items-center gap-2 text-neutral-300 hover:text-red-500 transition-colors">
                 <div className="p-3 bg-neutral-800 rounded-full">
